...
export default function PortalHeader() {
  ...
  return <header className="portal-header"><div className="container portal-nav"><Link to={profile?.role === 'staff' || profile?.role === 'admin' ? '/portal/staff' : '/portal/dashboard'} className="brand"><img src={logo} alt="Reliance Learning Centre crest"/><span>Reliance Learning Centre<small>Portal · My dashboard</small></span></Link><div className="portal-user"><PortalSiteExitLink className="portal-site-link">← Main website</PortalSiteExitLink><span>{profile?.full_name || 'Portal account'}{profile?.role && <small>{profile.role}</small>}</span><button className="btn secondary" onClick={handleLogout}>Log out</button></div></div></header>
}
