...
Deno.serve(async request => {
  ...
  try {
    ...
    if (profile?.role !== 'teacher') throw new Error('This setup is only available to teacher accounts.')
    const { data: duplicate } = await admin.from('profiles').select('id').eq('phone', nextPhone).neq('id', user.id).maybeSingle()
    if (duplicate) throw new Error('That phone number is already registered.')
    const { error: authError } = await admin.auth.admin.updateUserById(user.id, { email: portalEmail(nextPhone), password })
    if (authError) throw authError
    const { error: profileError } = await admin.from('profiles').update({ phone: nextPhone, must_update_credentials: false }).eq('id', user.id)
    if (profileError) throw profileError
    const { error: accountError } = await admin.from('staff_accounts').update({ phone_number: nextPhone }).eq('user_id', user.id)
